//
//  FCLayoutNode_Yoga_1_14.m
//  FlexboxCanvas
//
//  Created by Guang Yang on 2021/8/11.
//

#import "FCLayoutNode_Yoga_1_14.h"
#import <yoga/Yoga.h>

@interface FCLayoutNode_Yoga_1_14 ()
@property (nonatomic, readonly) YGNodeRef nodeRef;
@property (nonatomic, weak, nullable) id<FCLayoutMeasure> measurer;
@end

static YGSize FCLayoutNode_Yoga_1_14_measureFunc(YGNodeRef node, float width, YGMeasureMode widthMode, float height, YGMeasureMode heightMode) {
    FCLayoutNode_Yoga_1_14 *obj = (__bridge FCLayoutNode_Yoga_1_14 *)YGNodeGetContext(node);
    if (obj && obj.measurer) {
        CGSize cgSize = [obj.measurer measureInSize:CGSizeMake(width, height) widthMode:(FCLayoutMeasureMode)widthMode heightMode:(FCLayoutMeasureMode)heightMode];
...
@implementation FCLayoutNode_Yoga_1_14

+ (void)load {
    [FCLayoutNode registerClass:self];
}
...
- (void)setSubnodes:(NSArray<id<FCLayoutNode>> *)subnodes {
    ...
    if (subnodes) {
        for (FCLayoutNode_Yoga_1_14 *node in subnodes) {
...
- (void)setMeasurer:(id<FCLayoutMeasure>)measurer {
    _measurer = measurer;
    [self configMeasureFunc];
}
...
- (void)configMeasureFunc {
    if (_measurer && 0 == YGNodeGetChildCount(_nodeRef)) {
        if (!YGNodeHasMeasureFunc(_nodeRef)) {
            YGNodeSetMeasureFunc(_nodeRef, FCLayoutNode_Yoga_1_14_measureFunc);
        }
    } else {
        if (YGNodeHasMeasureFunc(_nodeRef)) {
